...
module.exports = {
  checkUserName(userName) {
    ...
    if (!regex.test(userName))
      throw "Error: userName must be only alphanumeric characters and no spaces";
  ...
  checkPostID(postId) {
    ...
    if (!Array.isArray(postId))
      throw "Error: postId must be an array of objects";
    postId.forEach((ele) => {
      ele = ele.trim();
      if (typeof ele !== "object")
        throw "Error: each element in postId must be an object";
    });
  ...
  checkPassword(password) {
    ...
    if (!regex.test(password))
      throw "Error: password must contain at least one uppercase character, one number and one special character";
