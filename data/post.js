...
const updatelikeCount = async(postId, userId, isLike) => {
    try{
        ...
        if(typeof postId !== 'string' || typeof userId !== 'string') throw 'Error: You must have userId and postId of type string';
        if (!ObjectId.isValid(userId)) throw "Error: The provided userID not a valid objectID";
        if (!ObjectId.isValid(postId)) throw "Error: The provided postId not a valid objectID";
        const postCollection = await posts();
        let likeCount = await getLikeCount(postId);

        if(isLike){
            likeCount++;
            await postCollection.updateOne({_id: ObjectId(postId)},{$set: {likes: likeCount}});
