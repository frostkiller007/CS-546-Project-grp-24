...
router
  ...
  .post(async (req, res) => {
    ...
    //TODO: validation
    try {
      const newUser = await usersData.createUser(usersData.username, usersData.email,
        usersData.age, usersData.city, usersData.state, usersData.postID,
        usersData.password, usersData.profilePicture);
      // if(newUser){
      ...
      return res.status(500).render('homepage/register',{error: 'Internal Server Error'});
    }
  })
...
router
  ...
  .post(async (req, res) => {
    ...
    try{
      const loginUser = await usersData.checkUser(usersData.usernameInput, usersData.passwordInput);
      if(loginUser){
        req.session.AuthCookie = userData.usernameInput;
        req.session.user = { Username: userData.usernameInput} 
        res.redirect('/homepage');
      }else{
        return res.status(401).render('homepage/login',{error:"Provide a valid username and/or password"});
      }
    }catch(e){
      return res.status(401).render('homepage/login',{error:"Provide a valid username and/or password"});
    }
